...
{% raw %} $$q_t^{(1)}(u_t,x_t) = \frac{1}{Z_t} \exp \left[  \sum_{ \textbf{u}_{\setminus t}, \textbf{x}_{\setminus t}} q_{\setminus t}(\textbf{u}_{\setminus t}, \textbf{x}_{\setminus t}) \ln p(\textbf{u}, \textbf{x} | \textbf{y}) \right] $$ {% endraw %}

or

{% raw %} $$q_t^{(1)}(u_t,x_t) = 
\frac{1}{Z_t} \exp \left[  \sum_{u_{t-1},x_{t-1} } q_{t-1}^{(0)} (u_{t-1}, x_{t-1}) \ln p(u_t,x_t | u_{t-1},x_{t-1}) \right] $$ {% endraw %}

{% raw %} $$ + \frac{1}{Z_t} \exp \left[  \sum_{u_{t+1}, x_{t+1} } q_{t+1}^{(0)}(u_{t+1},x_{t+1}) \ln p(u_{t+1},x_{t+1} | u_t, x_t) \right], $$ {% endraw %}
